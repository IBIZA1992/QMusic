...
@interface QMLikeDetailViewController ()<UIScrollViewDelegate, UIGestureRecognizerDelegate, AVAudioPlayerDelegate>
...
@property (nonatomic, assign) NSInteger musicArrayLikeIndex;
...
- (void)viewDidLoad {
    ...
    // 加载音乐(序号不一样，改序号)
    int countLikeMusic = 0;
    for (self.musicArrayIndex = 0; self.musicArrayIndex < self.musicArray.count; self.musicArrayIndex++) {
        NSDictionary *dict = self.musicArray[self.musicArrayIndex];
        if ([dict[@"like"] intValue]) {
            countLikeMusic++;
        }
        if (countLikeMusic == self.musicArrayLikeIndex + 1) {
...
- (void)backMusicClick {
    ...
    if (self.switchMusicIndex % 3 == 2) {  // 随机播放
        ...
        do {
            ...
            self.isLikeMusic = [dict[@"like"] intValue];
            i++;
            if (i == self.musicArray.count + 5) {
                self.player.currentTime = 0;
                break;
            }
        } while(!self.isLikeMusic);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    int page = self.scrollView.contentOffset.x / pageWidth;  // 判断第几页
    if (page == 0) {
